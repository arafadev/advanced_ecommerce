                    ...
                    <div class="row m-sm-0">
                        <div class="col-sm-2 p-sm-0 order-2 order-sm-1 mt-2 mt-sm-0">
                            <div class="owl-thumbs d-flex flex-row flex-sm-column" data-slider-id="1">
                                @foreach ($product->media as $media)
                                    <div class="owl-thumb-item flex-fill mb-2 mr-2 {{ !$loop->last ? 'mr-sm-0' : null }} ">
                                        <img class="w-100" src="{{ asset('assets/products/' . $media->file_name) }}"
                                            alt="...">
                                    </div>
                                @endforeach
                        ...
                        <div class="col-sm-10 order-1 order-sm-2">
                            <div class="owl-carousel product-slider" data-slider-id="1">
                                @foreach ($product->media as $media)
                                    <a class="d-block" href="{{ asset('assets/products/' . $media->file_name) }}"
                                        data-lightbox="product" title="{{ $product->name }}">
                        ...
                        @if ($product->reviews_avg_rating != '')
                            @for ($i = 0; $i < 5; $i++)
                                <li class="list-inline-item m-0"><i
                                        class="{{ round($product->reviews_avg_rating) <= $i ? 'far' : 'fas' }} fa-star fa-sm text-warning"></i>
                                </li>
                            @endfor
                        @else
                        ...
                        <li class="px-3 py-2 mb-1 bg-white text-muted">
                            ...
                            @foreach ($product->tags as $tag)
                                <a class="reset-anchor ml-2"
                                    href="{{ route('frontend.shop_tag', $tag->slug) }}">{{ $tag->name }}</a>
                            @endforeach
                        </li>
                    </ul>
                </div>
                            ...
                            <div class="col-lg-8">
                                @forelse($product->reviews as $review)
                                    <div class="media mb-3">
                                        <img class="rounded-circle" src="{{ asset('frontend/img/customer-1.png') }}"
                                            alt="" width="50">
                                                ...
                                                @if ($review->rating != '')
                                                    @for ($i = 0; $i < 5; $i++)
                                                        <li class="list-inline-item m-0">
                                                            <i
                                                                class="{{ round($review->rating) <= $i ? 'far' : 'fas' }} fa-star fa-sm text-warning"></i>
                                                        </li>
                                                    @endfor
                                                @else
                                                    <li class="list-inline-item m-0"><i
