      ...
      <ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">
          ...
          {{-- {{ dd($admin_side_menu) }}  // fetcing it from cache --}}
          @role(['admin'])
                          ...
                          <li
                              ...
                              @if ($menu->appearedChildren !== null && count($menu->appearedChildren) > 0)
                                  <div id="collapse_{{ $menu->route }}"
                                      ...
                                      aria-labelledby="heading_{{ $menu->route }}" data-parent="#accordionSidebar">
                                      <div class="bg-white py-2 collapse-inner rounded">
                                          @foreach ($menu->appearendChildren as $sub_menu)
                                              @permission($sub_menu->name)
                                                  <a class="collapse-item {{ getParentOf($current_page) != null && (int) (getParentIdOf($current_page) + 1) == $sum_menu->id ? 'active' : null }}"
                                                      href="{{ route('admin.' . $sub_menu->as) }}">{{ $sub_menu->display_name }}
                                                  </a>
                                              @endpermission
                                          @endforeach
                                      </div>
